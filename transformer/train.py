...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # During inference, we calculate the encoder output only once and reuse it for every token generated
    # by the decoder for a given english input sequence when translating to italian.
    encoder_output = model.encode(source, source_mask)
    # Initialize the decoder input with the sos token

    decoder_input = torch.empty(1, 1).fill_(sos_idx).type_as(source).to(device) # tensor([[2.]])
...
def train_model(config):
    ...
    # ignore_index is used to ignore the padding token in the loss calculation
    # and label_smoothing is used to prevent overfitting by smoothing the labels.
